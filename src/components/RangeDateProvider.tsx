/**
 * <RangeDateProvider> — provider for date-range selection.
 *
 * Mirrors the responsibilities of the legacy <CalendarProvider> but
 * accepts only the props that apply to a range pick. Single- and
 * multiple-mode-only props (`initialDate`, `initialDates`,
 * `maxSelected`) are not part of the type — TS rejects them at the
 * call site.
 *
 *   1. Construct a `RangeCalendarStore` once (kept in state).
...
export interface RangeDateProviderProps {
  /**
   ...
   * `useRangeCalendarActions().setActiveSystem(id)`. Unknown ids are
   * warned + ignored.
   */
  ...
  /** Called when `useRangeCalendarActions().confirm()` fires. */
  onConfirm?: RangeOnConfirm;
  /** Called when `useRangeCalendarActions().clear()` fires. */
  onClear?: RangeOnClear;
  /** Fires whenever the selection changes (any pick / clear). */
...
export function RangeDateProvider({
  ...
  const stableOnConfirm =
    useStableCallback<[RangeSelectionPayload]>(onConfirm);
  const stableOnClear = useStableCallback<[]>(onClear);
  const stableOnChange = useStableCallback<[RangeSelectionPayload]>(onChange);
