  ...
  return (
    <div className={wrapperCls}>
      ...
      >
        ...
        <span className={`${styles.perfCount}${hot ? ` ${styles.perfCountHot}` : ''}`}>
          {count.current}×
        </span>
      </button>
    </div>
...
export default function PerfCalendarDemo() {
  ...
  const onPress = useCallback((date: Date) => {
    const { rangeStart, rangeEnd } = selRef.current;
  ...
  const selText = rangeStart && rangeEnd
  ...
  return (
    <div className={styles.perfContainer}>
      ...
      <div className={styles.header}>
        <button className={styles.navButton} onClick={prevMonth} aria-label="Previous month">‹</button>
        <div className={styles.monthYear}>
          <span className={styles.month}>{MONTH_NAMES[month]}</span>
          <span className={styles.year}>{year}</span>
        </div>
        <button className={styles.navButton} onClick={nextMonth} aria-label="Next month">›</button>
      </div>
